...
| `src/corecycler/history/` | `db.py` (WAL SQLite, schema v15), `context.py`, `logger.py`, `export.py` |
...
- `README.md` regions between `<!-- BEGIN generated:X -->` / `<!-- END generated:X -->` (`badges`, `upstream`, `installation`, `options`, `footer`). The options block is rewritten by `scripts/update-readme-options.sh`.
- `.pre-commit-config.yaml` (0 bytes, gitignored, generated by git-hooks.nix through `flake.nix`).
- `scripts/update.sh`, `scripts/heal-overlays.sh`, `scripts/classify-build-failure.sh` are canonical copies synced from `nix-packaging-standard`. Change them upstream.
