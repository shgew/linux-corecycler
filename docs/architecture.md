...
src/corecycler/
  ...
  cli.py             Headless doctor/status/report/tune/resume commands (no display needed)
  ...
  tuner/             Automated PBO Curve Optimizer tuner
    ...
    bisect.py          Attribution hunt state machine: stock control, then bisection down to a lone reproducer
