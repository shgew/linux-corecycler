"""End-to-end fault injection for tuner crash and resume behavior.

The module exercises real persistence, live-mask search, resume,
attribution-hunt, journal, penalty, and fail-closed hardware paths. Workers,
monitors, stress binaries, and sleeps are replaced with deterministic queues.

Pinned invariants include:

* a hard crash with no in-test flag is caught by the CO write-ahead journal;
* repeated unattributed crash-resumes open a stock-control hunt rather than stop;
* unstable baselines retreat toward stock;
* SMU write and restoration failures stop safely;
* thermal protection fails closed when sensors are unavailable; and
...
def _synchronous_engine_callbacks():
    """Keep engine continuations inline and disable the real monitor thread."""
    with (
        patch("corecycler.tuner.engine.QTimer.singleShot", new=lambda _ms, fn: fn()),
        patch.object(TunerEngine, "_start_freeze_monitor"),
        patch.object(TunerEngine, "_stop_freeze_monitor"),
    ):
        yield


@pytest.fixture(autouse=True)
def _fault_reboots_are_unclean(monkeypatch):
    """Fault-injection resumes model a freeze unless a test says otherwise."""
    monkeypatch.setattr(engine_mod, "last_boot_ended_cleanly", lambda **_kwargs: False)
...
def _resume_fresh(db, topo, smu, backend, sid, **cfg_kwargs) -> TunerEngine:
    """Resume in a fresh engine without starting real worker threads."""
    eng = make_engine(db, topo, smu, backend, **cfg_kwargs)
    eng._start_worker = lambda *args, **kwargs: None
    eng._start_multi_core_worker = lambda *args, **kwargs: None
...
def _make_topo(n_cores: int, n_ccds: int):
    ...
    for i in range(n_cores):
        topo.cores[i] = PhysicalCore(core_id=i, ccd=min(i // per_ccd, n_ccds - 1), logical_cpus=(i,))
    topo.ccds = n_ccds
    return topo
...
def _stub_soak_worker(pending, stage_of):
    class _Stub(_StubThread):
        def __init__(self, core_id, _duration, thermal=None, parent=None):
            super().__init__()
...
def drive_validation(db, topo, backend, cliffs, agg_margin, cfg_kw, cap=8000, abort_at=0, faults=None):
    """Drive validation through its finite checks and into steady endurance.

    Cores are seeded CONFIRMED at their individual stable limit. The driver
    stops after the learned vector stays fixed through a long endurance window;
    validation intentionally has no terminal state anymore.

    Stages that construct worker classes directly are replaced with queueing
    stubs so no real thread or workload is started.
    """
    sid = db.create_tuner_session(TunerConfig(**cfg_kw).to_json(), "", "")
    ...
    with (
        ...
        while pending and steps < cap and settled_for < 200:
            steps += 1
...
def drive_closed_loop(
    ...
    """Drive the real live-mask loop; only worker execution is replaced."""
    world = {"rebooted": True}
    ...
    def solo(core_id, duration, **_kw):
        pending.append((frozenset({core_id}), core_id, float(duration)))

    def multi(cores, duration, **_kw):
        loaded = frozenset(cores)
        pending.append((loaded, min(loaded), float(duration)))

    def fresh():
        eng = make_engine(db, topo, smu, backend, **cfg_kw)
        eng._start_worker = solo
        eng._start_multi_core_worker = multi
        eng._session_id = sid
        return eng
    ...
    while pending and steps < cap and settled_for < 200:
        ...
        eng = holder["eng"]
        hard = [c for c in loaded if smu.applied.get(c, 0) <= cliffs[c][1]]
        soft = [c for c in loaded if smu.applied.get(c, 0) < cliffs[c][0]]
        if hard:
            crashes += 1
            pending.clear()
            smu.applied.clear()
            ...
            eng._on_test_finished(soft[0], False, "calc error", "computation", duration, 0.0)
        else:
            eng._on_test_finished(reporter, True, "", "", duration, 0.0)
        vector = holder["eng"].live_vector()
        settled_for = settled_for + 1 if vector == last_vector else 0
        last_vector = vector
    if (
        not pending
        and holder["eng"].status == "idle"
        and all(cs.phase is TunerPhase.SETTLED for cs in holder["eng"]._core_states.values())
        and holder["eng"].live_vector() == last_vector
    ):
        # A terminal settled search has no worker left to produce redundant
        # observations. It is stronger than 200 unchanged in-flight verdicts.
        settled_for = 200
    return holder["eng"], steps, crashes, sid, settled_for


def drive_intermittent(db, topo, backend, cliffs, flaky, cfg_kw, baseline=0, cap=8000):
    """Drive live-mask probes where marginal offsets fail only on a later visit."""
    sid = db.create_tuner_session(TunerConfig(**cfg_kw).to_json(), "", "")
    smu = FaultSMU()
    pending: list[tuple[frozenset[int], int, float]] = []
    ...
    while pending and steps < cap and settled_for < 200:
        ...
        culprit = reporter
        for core in loaded:
            offset = smu.applied.get(core, 0)
            stable, hard = cliffs[core]
            if offset <= hard:
                verdict, culprit = "crash", core
                break
            if offset < stable:
        ...
        if verdict == "crash":
            crashes += 1
            crashed_at[culprit].add(smu.applied.get(culprit, 0))
            pending.clear()
            smu.applied.clear()
            holder["eng"] = fresh()
            holder["eng"].resume(sid)
...
class TestJournalCatchesUnflaggedCrash:
    def test_unflagged_crash_is_penalized_via_journal(self, db, topo, smu, mock_backend):
        ...
        logs = db.get_tuner_test_log(sid, core_id=0)
...
class TestResumeCrashCircuitBreaker:
    ...
    def test_failed_stock_restoration_quarantines(self, db, topo, smu, mock_backend):
        ...
        for cs in eng._core_states.values():
            db.upsert_tuner_core_state(sid, cs)
        smu.applied = {0: -10, 1: -10}
        smu.reject_set = True
        messages = []
        eng.log_message.connect(messages.append)

        assert not eng._apply_co_mask(0, -20, Mask.LIVE)

        assert eng.status == "profile_quarantined"
        assert db.get_tuner_session(sid).status == "profile_quarantined"
        assert smu.applied == {0: -10, 1: -10}
        assert all(not cs.in_test for cs in eng._core_states.values())
        assert any("Stock restoration failed" in message for message in messages)

    def test_clean_resume_does_not_increment_breaker(self, db, topo, smu, mock_backend):
        """A normal pause/resume with no crash does not arm an attribution hunt."""
    ...
    def test_surviving_test_resets_breaker(self, db, topo, smu, mock_backend):
        """A surviving test resets the streak so old crashes do not trigger a hunt."""
...
class TestReopeningAQuarantinedSession:
    ...
    def test_unproven_inherited_baseline_is_sanitized_to_survived_value(self, db, topo, smu, mock_backend):
        sid = self._quarantined(db, current=-30, baseline=-25, survived=-10)

        _resume_fresh(db, topo, smu, mock_backend, sid, cores_to_test=[0])

        cs = db.get_tuner_core_states(sid)[0]
        assert (cs.current_offset, cs.baseline_offset, cs.best_offset) == (-10, -10, -10)

    def test_quarantine_reopen_falls_back_to_stock_without_survival_evidence(self, db, topo, smu, mock_backend):
        sid = self._quarantined(db, current=-25, baseline=-20)

        _resume_fresh(db, topo, smu, mock_backend, sid, cores_to_test=[0])

        cs = db.get_tuner_core_states(sid)[0]
        assert (cs.current_offset, cs.baseline_offset, cs.best_offset) == (0, 0, 0)

    def test_quarantine_reopen_retains_proven_state(self, db, topo, smu, mock_backend):
        sid = self._quarantined(db, current=-10, baseline=-5, survived=-10)

        _resume_fresh(db, topo, smu, mock_backend, sid, cores_to_test=[0])

        cs = db.get_tuner_core_states(sid)[0]
        assert (cs.current_offset, cs.baseline_offset, cs.best_offset) == (-10, -5, -10)
...
class TestValidationCrashArmsBreaker:
    ...
    def _seed_validating_at_stage2(self, db, smu, topo, backend, cliffs, **cfg):
        ...
        sid = db.create_tuner_session(TunerConfig(cores_to_test=list(cliffs), **cfg).to_json(), "", "")
...
class TestSMUWriteFault:
    def test_rejected_write_pauses_and_journals_intent(self, db, topo, smu, mock_backend, monkeypatch):
        """A rejected candidate write is journaled before the attempt, then the
        actual hardware is restored to baseline before the tuner pauses."""
        eng = make_engine(db, topo, smu, mock_backend, cores_to_test=[0])
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        eng._core_states = {
            0: CoreState(core_id=0, phase=TunerPhase.COARSE_SEARCH, current_offset=-10, baseline_offset=0)
        }

        journal_before_writes = []

        def reject_candidate(core_id, value):
            journal_before_writes.append(
                (value, db.journal_values(eng._session_id), db.journal_suspects(eng._session_id))
            )
            smu.writes.append((core_id, value))
            if value == -10:
                return False
            smu.applied[core_id] = value
            return True

        monkeypatch.setattr(smu, "set_co_offset", reject_candidate)
        ok = eng._apply_co_mask(0, -10, Mask.LIVE)

        assert ok is False
        assert eng._status == "paused"
        assert smu.writes == [(0, -10)] * 3 + [(core_id, 0) for core_id in range(4)]
        assert journal_before_writes[:3] == [(-10, {0: -10}, [(0, -10)])] * 3
        for core_id, (value, journal, suspects) in enumerate(journal_before_writes[3:]):
            assert value == 0
            assert journal[core_id] == 0
            assert suspects == []
        assert smu.applied[0] == 0
        assert db.journal_values(eng._session_id) == dict.fromkeys(topo.cores, 0)
        assert db.journal_suspects(eng._session_id) == []

    def test_raising_write_quarantines_when_stock_cannot_be_verified(self, db, topo, smu, mock_backend):
        eng = make_engine(db, topo, smu, mock_backend, cores_to_test=[0])
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        eng._core_states = {
            0: CoreState(core_id=0, phase=TunerPhase.COARSE_SEARCH, current_offset=-10, baseline_offset=0)
        ...
        ok = eng._apply_co_mask(0, -10, Mask.LIVE)
        assert ok is False
        assert eng._status == "profile_quarantined"
...
class TestWriteAheadJournal:
    ...
    def test_within_envelope_value_journaled_survived(self, db, topo, smu, mock_backend):
        ...
        eng._write_co_verified(0, -20)  # less aggressive than proven
        assert db.journal_suspects(eng._session_id) == []  # not a suspect

    def test_zero_is_always_survived(self, db, topo, smu, mock_backend):
        eng = make_engine(db, topo, smu, mock_backend, cores_to_test=[0])
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        eng._write_co_verified(0, 0)
        assert db.journal_suspects(eng._session_id) == []
        assert db.journal_survived_values(eng._session_id).get(0) == 0

    def test_out_of_range_offset_is_rejected_before_journaling(self, db, topo, smu, mock_backend):
        smu.commands.co_range = (-50, 10)
        eng = make_engine(db, topo, smu, mock_backend, cores_to_test=[0])
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        assert eng._write_co_verified(0, -51) is False
        assert db.journal_values(eng._session_id) == {}
    ...
    def test_resume_rebuilds_proven_safe_envelope_from_journal(self, db, topo, smu, mock_backend):
        ...
        with (
            ...
            patch.object(engine_mod, "last_boot_ended_cleanly", return_value=True),
        ):
            eng.resume(sid)
    ...
    def test_apply_co_without_smu_returns_false(self, db, topo, mock_backend):
        ...
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        assert eng._write_co_verified(0, -10) is False
...
class TestClosedLoopSimulation:
    ...
    def test_converges_safely_under_injected_crashes(self, db, topo, mock_backend, order):
        ...
        eng, steps, crashes, sid, settled_for = drive_closed_loop(db, topo, mock_backend, cliffs, cfg_kw)
...
class TestIntermittentInstability:
    """Exercise an offset that passes once and hard-crashes on a later probe.

    The learned live vector must retreat beyond every offset that actually
    crashed; the search itself remains active because annealing never declares
...
class TestInterruptionSafety:
    def test_abort_reverts_all_cores_not_just_the_tested_one(self, db, topo, smu, mock_backend):
        ...
        eng._session_id = db.create_tuner_session(TunerConfig(cores_to_test=[0, 1, 2]).to_json(), "", "")
        eng._core_states = {
...
class TestPropertyFuzz:
    ...
    def test_tuner_robust_over_random_scenarios(self, data):
        ...
        try:
            ...
            eng, steps, crashes, sid, _settled_for = drive_closed_loop(
                db,
                topo,
                _StubBackend(),
                cliffs,
            ...
            if eng.status == "paused":
                ...
                assert db.get_tuner_session(sid).status == "paused"
            else:
                assert eng.status in ("running", "idle")
            resident = eng.live_vector()
            for c, (_stable, crash) in cliffs.items():
                r = resident.get(c, 0)
...
class TestForwardCrashWriteAhead:
    ...
    def test_journal_is_durable_to_a_fresh_connection(self, tmp_path, topo, mock_backend):
        ...
        eng._session_id = db1.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        eng._write_co_verified(0, -30)
...
class TestValidationFuzz:
    """Validation reaches a stable safe vector before continuing endurance."""

    @settings(max_examples=200, deadline=None, suppress_health_check=[HealthCheck.too_slow])
    @given(data=st.data())
    def test_validation_settles_safely_before_endurance(self, data):
...
class TestAbortSafety:
    ...
    def test_abort_mid_validation_reverts_every_core(self, data):
        ...
        try:
            ...
            assert db.get_tuner_session(eng._session_id).status == "aborted"
        finally:
            db.close()
...
class TestResumePathsValidateConfig:
    ...
    def test_resume_fails_closed_on_out_of_range_config(self, db, topo, smu, mock_backend):
        sid = db.create_tuner_session(TunerConfig(cores_to_test=[0], coarse_step=0).to_json(), "", "")
...
class TestRebootGate:
    ...
    def test_config_override_cannot_hide_a_reboot(
        ...
        with patch.object(db, "_now_iso", return_value=old):
            sid = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
            db.upsert_tuner_core_state(
                sid, CoreState(core_id=0, phase=TunerPhase.COARSE_SEARCH, current_offset=-30, in_test=True)
            )
        ...
        db.update_tuner_session_config(sid, TunerConfig(cores_to_test=[0], endurance=True).to_json())
    ...
    def test_boot_identity_overrides_wall_clock_and_metadata(
        ...
        with patch.object(db, "_now_iso", return_value=timestamp):
            ...
            tp.journal_co_intent(db, sid, 0, -30, survived=False)
        db.set_session_boot(sid, previous_boot)
        db.update_tuner_session_config(sid, TunerConfig(cores_to_test=[0], endurance=True).to_json())
    ...
    def test_unreadable_forensics_preserves_recovery_until_it_can_be_read(
        ...
        monkeypatch.setattr(engine_mod, "_rebooted_since", assume_rebooted)
        sid = db.create_tuner_session(TunerConfig(cores_to_test=[0]).to_json(), "", "")
        db.set_session_boot(sid, "old-boot")
        db.upsert_tuner_core_state(
            sid, CoreState(core_id=0, phase=TunerPhase.COARSE_SEARCH, current_offset=-30, in_test=True)
        )
        tp.journal_co_intent(db, sid, 0, -30, survived=False)
        ...
        assert smu.writes == []
        assert db.get_tuner_session(sid).boot_id == "old-boot"
        ...
        assert db.get_tuner_session(sid).boot_id == "test-boot"
    ...
    def test_rebooted_since_compares_boot_identity(self, assume_rebooted):
        _rebooted_since = assume_rebooted
        assert _rebooted_since("ignored", previous_boot_id="same", boot_id="same") is False
        assert _rebooted_since("ignored", previous_boot_id="old", boot_id="new") is True
...
# ---------------------------------------------------------------------------
# A hard crash at a CONFIRMED value invalidates the confirmation
# ---------------------------------------------------------------------------
...
class TestCrashAtConfirmedValue:
    def test_crash_demotes_best_and_reenters_backoff(self, db, topo, smu, mock_backend):
        ...
        tp.journal_co_intent(db, sid, 0, -42, survived=False)

        crashed, pending_hunt = eng._attribute_crash_after_reboot(db.get_tuner_session(sid))
        assert crashed == [0]
...
class TestStartupFailureIsNotAVerdict:
    def test_startup_failure_pauses_reverts_and_persists(self, db, topo, smu, mock_backend):
        ...
        assert db.get_tuner_test_log(sid) == []  # no verdict recorded
    ...
    def test_validation_env_failure_pauses_without_backoff(self, db, topo, smu, mock_backend):
        ...
        cliffs = {0: -10, 1: -12}
        sid = db.create_tuner_session(TunerConfig(cores_to_test=list(cliffs)).to_json(), "", "")
        ...
        assert all(r["passed"] is not False or r["error_type"] != "startup" or True for r in db.get_tuner_test_log(sid))
        # no validate FAIL verdict was recorded
        assert not [r for r in db.get_tuner_test_log(sid) if r["phase"].startswith("validate")]
...
class TestApparatusBreaker:
    # BACKOFF_PRECONFIRM starts with the first boost workload in the battery.
    BASE = dict(
        backend="mprime",
        stress_mode="SSE",
        fft_preset="SMALL",
    ...
    def _seed(self, db, topo, smu, backend, streak_threshold=5, **core):
        sid = db.create_tuner_session(
            TunerConfig(cores_to_test=[0], apparatus_failure_streak=streak_threshold).to_json(),
            "",
            "",
        )
        eng = make_engine(
    ...
    def test_synthetic_crash_rows_do_not_count(self, db, topo, smu, mock_backend):
        ...
        for off in (-22, -21):
            db.insert_tuner_test_log(
                sid, 0, off, "coarse_search", False, error_type="crash", duration=None, **self.BASE
            )
        with patch.object(eng, "_run_next"):
            eng._on_test_finished(0, False, "mprime error: FATAL ERROR", "computation", 122.0, 0.0)
    ...
    def test_fresh_regime_has_no_prior_pass_to_contradict(self, db, topo, smu, mock_backend):
        eng, sid, _ = self._seed(db, topo, smu, mock_backend, streak_threshold=3)
        boost = {**self.BASE, "regime": str(Regime.BOOST)}
        current = {**self.BASE, "regime": str(Regime.CURRENT)}
        self._log(db, sid, -20, "confirm", True, workload=boost)
        for off in (-19, -18, -17):
            self._log(db, sid, off, "backoff_preconfirm", False, workload=current)

        assert eng._apparatus_suspect(0) is False
        assert eng.status != "paused"
...
class TestApparatusFaultIsNotAVerdict:
    ...
    def test_stall_in_search_flow_retries_same_offset(self, db, topo, smu, mock_backend):
        ...
        assert db.get_tuner_test_log(sid) == []
...
class TestFailClosedFinalize:
    ...
    def test_finalize_exhausted_reverts_and_pauses(self, db, topo, smu, mock_backend):
        ...
        assert db.get_tuner_session(sid).status != "completed"
...
class TestUnattributedIncidentOnResume:
    """An unexplained validation reboot starts attribution instead of pausing."""
    ...
    def _seed(self, db, cliffs, unattributed=0):
        ...
        for c, v in cliffs.items():
            ...
            db.insert_tuner_test_log(sid, c, v, "confirm", True, duration=1.0)
        db.update_tuner_session_status(sid, "validating")
        if unattributed:
    ...
    def test_clean_reboot_mid_validation_is_not_an_incident(self, db, topo, smu, mock_backend, monkeypatch):
        ...
        assert db.get_unattributed_crashes(sid) == 0
        assert db.get_tuner_session(sid).hunt_state == ""
        assert eng._status == "validating"
